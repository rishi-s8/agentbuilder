...
class ExecuteToolsAction(Action):
    ...
    def run(self):
        ...
        for tool_call in self.tool_calls:
            ...
            self.conversation_wrapper.add_tool_message(tool_call_id, function_name, result_content)
...
class MakeLLMRequestAction(Action):
    ...
    def run(self):
        ...
        tools = [tool.to_openai_format() for tool in self.tool_map.values()] if self.tool_map else None
        ...
        if self.verbose:
            if assistant_message.tool_calls:
                ...
                print(f"   ✓ Response: {assistant_message.content[:100] if assistant_message.content else 'No content'}...")
...
class EmptyAction(Action):
    ...
    def run(self):
        ...
        if self.verbose:
            print(f"✅ Completed (empty conversation) in {self.iterations} iteration(s)\n")
        return ""
