...
class TestAgenticPlanner:
    ...
    def test_step_assistant_message_with_tool_calls(self, planner):
        ...
        tool_calls = [
            {"id": "call_123", "function": {"name": "add", "arguments": '{"x": 1, "y": 2}'}}
        ]
        history = [
    ...
    def test_step_verbose_mode(self, mock_conversation_wrapper, sample_tool_map, capsys):
    ...
    def test_step_multiple_tool_calls(self, planner):
        ...
        tool_calls = [
            {"id": "call_1", "function": {"name": "add", "arguments": '{"x": 1, "y": 2}'}},
            {"id": "call_2", "function": {"name": "add", "arguments": '{"x": 3, "y": 4}'}}
        ]
        history = [
    ...
    def test_step_complex_conversation_flow(self, planner, mock_conversation_wrapper, sample_tool_map):
        ...
        history.append(ToolMessageAction(tool_call_id="call_1", name="add", content='{"success": true, "data": 8}'))
        action = planner.step(history, iterations=3)
        assert isinstance(action, MakeLLMRequestAction)
